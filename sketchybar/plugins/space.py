...
ICON_MAP = [
    ...
    {"regex": r"Alacritty|Hyper|iTerm2|kitty|Terminal|Warp|WezTerm", "icon": ":terminal:"},
...
spaces = {}
all_spaces = json.loads(os.popen('yabai -m query --displays').read())[0]['spaces']
clear_spaces = {x: True for x in all_spaces}

apps = json.loads(os.popen('yabai -m query --windows').read())
for app in apps:
    clear_spaces[app['space']] = False
...
clear = ' '.join([f'--set space.{space} label.drawing=off' for space, clear in clear_spaces.items() if clear])
default_args = '--set \'/space\\..*/\' background.drawing=on --animate sin 10'

os.system(f'sketchybar -m {default_args} {args} {clear}')
